...
impl BufferSizeAdapter {
    pub fn new(runtime_block_size: usize) -> Self {
        Self {
            ring_buffer: VecDeque::with_capacity(4 * MAX_HOST_FRAMES), // Increased capacity
            runtime_block_size,
    ...
    pub fn fill_host_buffer(&mut self, host_buffer: &mut [f32], runtime: &mut Runtime, channels: usize) -> Result<(), &'static str> {
        ...
        while host_idx < host_buffer.len() {
            if self.ring_buffer.len() < self.runtime_block_size {
                // Process a block
                runtime.process_block(&mut self.block_buffer)?;
                self.ring_buffer.extend(&self.block_buffer);
            }
            // Take from ring buffer
            if let Some(sample) = self.ring_buffer.pop_front() {
